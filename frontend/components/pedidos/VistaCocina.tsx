...
/**
 * Vista de cocina: muestra pedidos pendientes de preparación y en preparación,
...
const KITCHEN_STATUSES: OrderStatus[] = ["pending_preparation", "in_preparation"]

export default function VistaCocina({ comercioId, userRole }: Props) {
    ...
    async (showRefreshing = false) => {
      ...
      try {
        const results = await Promise.all(
          KITCHEN_STATUSES.map((status) => api.pedidos.listar(comercioId, { status, page_size: 50 })),
        )
        const byStatus: Record<string, OrderListItem[]> = {}
        KITCHEN_STATUSES.forEach((status, i) => {
  ...
  const allEmpty = KITCHEN_STATUSES.every((s) => (pedidosPorEstado[s]?.length ?? 0) === 0)
  ...
  return (
    <div className="space-y-6">
      ...
      {allEmpty ? (
        <div className="text-center py-16 text-gray-400">No hay pedidos en cocina.</div>
      ) : (
        KITCHEN_STATUSES.map((status) => {
          ...
          const canAdvance =
            !!nextSt &&
            (userRole === "cook" || ["cashier", "admin", "owner"].includes(userRole))
                ...
                {items.map((pedido) => (
                  ...
                  >
                    ...
                    <ul className="text-sm space-y-0.5">
                      {pedido.items_summary.length > 0 ? (
                        pedido.items_summary.map((item, i) => (
                          <li key={i} className="text-brown">
                            {item}
                          </li>
                        ))
                      ) : (
                        <li className="text-gray-400 italic">Sin items</li>
                      )}
                    </ul>
